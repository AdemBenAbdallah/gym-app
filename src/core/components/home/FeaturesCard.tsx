...
import { Card, SimpleGrid, Stack, Text, Title, rem } from "@mantine/core";
...
export function FeaturesCards() {
  const features = mockdata.map((feature) => (
    <Card key={feature.title} shadow="md" radius="md" className={classes.card} padding="xl">
      <feature.icon style={{ width: rem(50), height: rem(50) }} stroke={2} color={"var(--mantine-color-custom-6)"} />
  ...
  return (
    <Stack px={100} mt={{ base: 100, md: 200 }}>
      <Vertical gap={0}>
        <Text> Services</Text>
        <Title fz={{ base: rem(35), md: rem(50) }}>Nos Services</Title>
      </Vertical>
      <SimpleGrid cols={{ base: 1, md: 4 }} spacing="xl" mt={{ base: 10, md: 50 }}>
        {features}
      </SimpleGrid>
    </Stack>
  );
